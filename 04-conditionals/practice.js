...
var interjectAt = function (a, b, c) {
    if (typeof (a) === "string" && typeof (b) === "number" && typeof (c) === "string") {
        if (c.length <= b) {
            throw new Error("the string doesn't have that many letters!");
        }
        else {
            var beginning = c.slice(0, b);
            var end = c.slice(b, c.length);
            // console.log(beginning);
            return ${ beginning } -${ a } -${ end };
        }
    }
    else {
        throw new Error("expected first arg to be a string, second arg to be a number and third arg to be a string");
    }
};
...
var randomInterject = function (a) {
    var messages = ["hello", "how", "why"];
    var randomIndex = Math.floor(Math.random() * 2);
    var randmonString = messages[randomIndex];
    var randomLocation = Math.floor(Math.random() * (a.length - 1))
    return interjectAt(a, randomLocation, randmonString);
};
